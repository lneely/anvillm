...
**After**:
```
execute_code with code: "bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) 82b93a8a"
```
...
for agent_id in agent-1 agent-2 agent-3; do
  bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) \$agent_id | wc -l
...
```
execute_code with code: "
inbox=\$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) 82b93a8a)
...
**Bad**:
```bash
result=$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) 82b93a8a)
...
**Bad**:
```bash
result1=$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) agent-1)
echo "$result1"
result2=$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) agent-2)
echo "$result2"
result3=$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) agent-3)
echo "$result3"
```
...
```bash
for agent_id in agent-1 agent-2 agent-3; do
  result=$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) "$agent_id")
  count=$(echo "$result" | wc -l)
...
**Bad**:
```bash
result=$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) invalid-id)
echo "$result"  # May fail silently
```

**Good**:
```bash
if result=$(bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) invalid-id 2>&1); then
  echo "$result"
else
...
```bash
for i in {1..10}; do
  tool=$(9p read anvillm/tools/anvilmcp/check_inbox.sh)
  # Parse and use...
done
```
...
for i in {1..10}; do
  bash <(9p read anvillm/tools/anvilmcp/check_inbox.sh) 82b93a8a
done
```
