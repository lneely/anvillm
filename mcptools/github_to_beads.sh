...
if 9p read "agent/beads/$mount/list" | jq -e ".[] | select(.title | contains(\"#$issue\"))" >/dev/null 2>&1; then
...
# Create parent bead
echo "new \"$bead_title\" \"$body\"" | 9p write "agent/beads/$mount/ctl"

# Get created bead ID
bead_id=$(9p read "agent/beads/$mount/list" | jq -r ".[] | select(.title | contains(\"#$number\")) | .id" | head -1)

# Update issue type if not task
if [[ "$issue_type" != "task" ]]; then
    echo "update $bead_id issue_type $issue_type" | 9p write "agent/beads/$mount/ctl"
fi

# Update status if closed
if [[ "$status" == "closed" ]]; then
    echo "update $bead_id status closed" | 9p write "agent/beads/$mount/ctl"
fi
...
if [[ -n "$body" ]]; then
    ...
    echo "$body" | grep -E '^\s*-\s+\[\s+\]' | sed -E 's/^\s*-\s+\[\s+\]\s*//' | while IFS= read -r task; do
        if [[ -n "$task" ]]; then
            echo "new \"$task\" \"\" $bead_id" | 9p write "agent/beads/$mount/ctl"
        fi
    done
fi
