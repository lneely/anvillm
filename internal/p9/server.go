...
func (s *Server) walk(cs *connState, fc *plan9.Fcall) *plan9.Fcall {
	...
	for _, name := range fc.Wname {
		...
		if path == "/" {
			...
			// Flat skill files: <name>.md
			if s.skills != nil {
				skills, _ := s.skills.listAllSkills()
				found := false
				for _, skill := range skills {
					if name == skill.Name+".md" {
						qid = plan9.Qid{Type: QTFile, Path: qidSkillsBase + hashID(name)}
						newPath = "/skills/" + name
						found = true
						break
					}
			...
			// Flat role files: <name>.md
			if s.roles != nil {
				roles, _ := s.roles.listAllRoles()
				found := false
				for _, role := range roles {
					if name == role.Name+".md" {
						qid = plan9.Qid{Type: QTFile, Path: qidRolesBase + hashID(name)}
						newPath = "/roles/" + name
						found = true
						break
					}
...
func (s *Server) readDir(path string, offset uint64, count uint32) []byte {
	...
	if path == "/" {
		...
		if s.skills != nil {
			// Flat: list <name>.md files
			skills, _ := s.skills.listAllSkills()
			for _, skill := range skills {
				dirs = append(dirs, plan9.Dir{
					Qid:  plan9.Qid{Type: QTFile, Path: qidSkillsBase + hashID(skill.Name + ".md")},
					Mode: 0444, Name: skill.Name + ".md", Uid: "q", Gid: "q", Muid: "q",
				})
			}
		}
	} else if path == "/roles" {
		if s.roles != nil {
			...
			for _, role := range roles {
				dirs = append(dirs, plan9.Dir{
					Qid:  plan9.Qid{Type: QTFile, Path: qidRolesBase + hashID(role.Name + ".md")},
